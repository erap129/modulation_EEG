...
while True:
    ...
    print(f'the 10 value is {np.average(ps[np.where((freqs >= 7) & (freqs <=10))])/1e16}')
    if calibrate_times > 0:
        avg_15 += np.average(ps[np.where((freqs >= 14) & (freqs <=16))])
        avg_10 += np.average(ps[np.where((freqs >= 7) & (freqs <= 10))])
    elif calibrate_times == 0:
        avg_15 /= 5
        avg_10 /= 5
        print(f'the threshold for 15 is {avg_15}')
        print(f'the threshold for 10 is {avg_10}')

    if calibrate_times < 0:
        if np.average(ps[np.where((freqs >= 14) & (freqs <=16))]) >= avg_15 * 1.5:
            print('I recognized the 15Hz frequency')
            send_event(url='http://7e4e54a2.ngrok.io/look')
        elif np.average(ps[np.where((freqs >= 7) & (freqs <=10))]) >= avg_10 * 1.5:
            print('I recognized the 10Hz frequency')
            send_event(url='http://7e4e54a2.ngrok.io/wake')
        else:
            print('I did"t recognize any frequency')
